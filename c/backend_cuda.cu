...
static DeviceContext g_ctx[ILI_CUDA_MAX_DEVICES];
static int g_nctx;
...
extern "C" int ili_cuda_init(const int *devices, int count) {
    int available = 0;
    if (!devices || count < 1 || count > ILI_CUDA_MAX_DEVICES) return 0;
...
extern "C" void ili_cuda_shutdown(void) {
...
extern "C" int ili_cuda_device_count(void) { return g_nctx; }

extern "C" int ili_cuda_device_at(int index) {
    return index >= 0 && index < g_nctx ? g_ctx[index].device : -1;
}

extern "C" int ili_cuda_mem_info(int device, size_t *free_bytes, size_t *total_bytes) {
    DeviceContext *ctx = find_ctx(device);
    if (!free_bytes || !total_bytes || !select_ctx(ctx)) return 0;
    return cuda_ok(cudaMemGetInfo(free_bytes, total_bytes), "memory info");
}

extern "C" void ili_cuda_stats(int device, size_t *tensor_count, size_t *tensor_bytes) {
...
extern "C" int ili_cuda_tensor_upload(IliCudaTensor **tensor,
    ...
    if (*tensor) {
        IliCudaTensor *t = *tensor;
        return t->fmt == fmt && t->I == I && t->O == O && t->device == device;
    }
    IliCudaTensor *t = static_cast<IliCudaTensor *>(std::calloc(1, sizeof(*t)));
    if (!t) return 0;
...
extern "C" int ili_cuda_matmul(IliCudaTensor **tensor,
                                 float *y, const float *x,
                                 const void *weights, const float *scales,
                                 int fmt, int S, int I, int O, int device) {
    if (S < 1 || !ili_cuda_tensor_upload(tensor, weights, scales, fmt, I, O, device)) return 0;
    IliCudaTensor *t = *tensor;
...
extern "C" void ili_cuda_tensor_free(IliCudaTensor *tensor) {
    if (!tensor) return;
...
extern "C" size_t ili_cuda_tensor_bytes(const IliCudaTensor *tensor) {
    return tensor ? tensor->weight_bytes + (tensor->fmt ? (size_t)tensor->O * sizeof(float) : 0) : 0;
}

extern "C" int ili_cuda_tensor_device(const IliCudaTensor *tensor) {
