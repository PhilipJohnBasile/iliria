...
"""Hardware and model placement planning for iliria's disk/RAM/VRAM tiers."""
...
def environment_for_plan(plan, env=None, cuda_enabled=True):
    ...
    def _get(name, default=None):
        # silent legacy fallback: ILI_* > COLI_* > FA_*
        for prefix in ("ILI_", "COLI_", "FA_"):
            if prefix + name in result:
    ...
    if _get("CUDA") is None:
        result["ILI_CUDA"] = "1"
    if _get("GPU") is None and _get("GPUS") is None:
        key = "ILI_GPU" if len(devices) == 1 else "ILI_GPUS"
