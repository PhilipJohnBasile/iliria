...
# ---- safeguard: ILI_METAL_PREFILL without ILI_METAL=1 (footgun: silently runs all-CPU
# instead of erroring -- see tools/effective_flags.py's module docstring). Real runs only;
# --mock never execs a real engine so there is nothing to protect. Centralized in ONE
# python module so this check can never drift from eval_glm.py's own guard or from each
# other -- see tools/effective_flags.py's header for why.
if [[ "$MOCK" == 0 ]]; then
  GUARD_MSG="$(python3 "$CDIR/tools/effective_flags.py" check-metal-prefill-guard 2>&1)" || die "$GUARD_MSG"
fi
...
telemetry_loop() {
  ...
  while true; do
    ...
    if [[ "$phase" == cold || "$phase" == steady ]]; then
      ...
      # ONE combined iostat call reports BOTH disk0 MB/s and cpu id% as live 1s deltas
      # (-c 2, last line -- the single-report form is the since-boot average, the bug fixed
      # in timing_watchdog.read_disk_mbs). Combined because two SEQUENTIAL -c 2 calls block
      # ~2s each, pushing one sampler iteration past the mock e2e test's 3s steady window
      # (test_roofline_run saw 0 steady samples -- the row-19 stop condition, 2026-07-16).
      iostat_out="$(iostat -w 1 -c 2 disk0 2>/dev/null)"
      ...
      mbcol="$(echo "$iostat_out" | awk '/MB\/s/{for(i=1;i<=NF;i++) if($i=="MB/s"){print i; exit}}' | head -1)"
      disk_mbs=""
      [[ -n "$mbcol" ]] && disk_mbs="$(echo "$iostat_out" | tail -1 | awk -v c="$mbcol" '{print $c}')"
