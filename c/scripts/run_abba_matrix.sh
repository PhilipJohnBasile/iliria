...
# ---- safeguard: ILI_METAL_PREFILL without ILI_METAL=1 in the AMBIENT environment
# (footgun: silently runs all-CPU instead of erroring -- see tools/effective_flags.py's
# module docstring). Real runs only; --dry-run never execs a real glm. Note: each arm below
# explicitly sets ILI_METAL_PREFILL=$mp itself, so this specifically catches a stray
# misconfigured ILI_METAL_PREFILL already sitting in the caller's shell (e.g. left over
# from a previous experiment) without ILI_METAL=1 also present -- the same mechanical
# check as eval_glm.py's, from the same centralized module, so it can never drift.
if [[ "$DRY_RUN" == 0 ]]; then
