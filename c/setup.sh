...
# iliria — installazione su una macchina nuova (Linux x86-64, macOS, Windows/MinGW).
# Compila il motore e fa un self-test. Il MODELLO (~372 GB int4) va copiato a parte
# o rigenerato con: ili convert --model <dir-su-ext4/NVMe>
set -e
cd "$(dirname "$0")"
echo "🤖 iliria — setup"
...
echo "  ./ili build           # already done"
echo "  ./ili convert --model /path/on/NVMe/glm52_i4     # generate the int4 model (hours)"
echo "  ./ili info  --model /path/on/NVMe/glm52_i4"
echo "  ./ili chat  --model /path/on/NVMe/glm52_i4 --ram <GB>"
echo
