...
/* byte size of one element for a given dtype code (st_dtype_code() above) -- used
 * by st_init() to cross-check a tensor's declared numel (shape product) against
 * its declared byte range (data_offsets), catching a malformed/hostile header
 * BEFORE st_read_f32()/st_read_raw() ever trust it: st_read_f32's BF16/F16 arm
 * walks `numel` elements over a `nbytes`-sized buffer (OOB READ if numel*elsize >
 * nbytes), and its F32 arm memcpy's `nbytes` bytes into a caller buffer sized for
 * `numel` floats (OOB WRITE if nbytes > numel*4) -- neither arm re-checks the
 * relationship itself, both simply trust st_tensor's fields. dtype==3 (U8/I8:
 * this container's quantized/packed weights AND mode-1.5 MH01 blobs) is always
 * declared shape=[nbytes] by this codebase's own convention (one byte is one
 * "element" -- see e.g. tests/test_mode15_*.c's own fixture writers), so
 * elemsize 1 is the correct, exact expectation there too, not just for the float
 * dtypes. */
static int st_dtype_size(int dtype) {
    if (dtype == 0) return 2;   /* BF16 */
...
static void st_init(shards *S, const char *snap_dir) {
    ...
    for (int fi = 0; fi < nf; fi++) {
        ...
        /* Physical file size, fetched once per shard: every tensor's ABSOLUTE byte
         * range [data_start+a0, data_start+b0) must land inside [data_start,
         * file_size) -- never before the data section (back into the JSON header)
         * and never past the file's real end. A malformed/hostile header whose
         * data_offsets are merely WITHIN the physical file (so pread() itself would
         * "succeed" -- see the pread-return-value checks here and in
         * st_read_f32()/st_read_raw()) but land on the WRONG bytes (another
         * tensor's, the header, or trailing garbage) would otherwise go
         * undetected until (if ever) that specific tensor is actually read this
         * run -- checked once, eagerly, for every declared tensor here instead. */
        struct stat fst;
        if (fstat(fd, &fst) != 0) { perror(files[fi]); exit(1); }
        int64_t file_size = (int64_t)fst.st_size;
        char *arena = NULL;
        ...
        for (int i = 0; i < root->len; i++) {
            ...
            /* Fail-closed on a malformed/hostile header -- mirrors st_dtype_code()'s
             * own exit(1)-on-unknown-dtype discipline just above. See
             * st_dtype_size()'s comment for exactly which OOB read/write each of
             * these three checks prevents downstream. */
            if (a0 < 0 || b0 < a0) {
                fprintf(stderr, "st_init: %s: tensor '%s' has invalid data_offsets [%lld,%lld]\n",
                        files[fi], name, (long long)a0, (long long)b0);
                exit(1);
            }
            int64_t nbytes = b0 - a0;
            int64_t want_nbytes = numel * (int64_t)st_dtype_size(dtype);
            if (nbytes != want_nbytes) {
                fprintf(stderr, "st_init: %s: tensor '%s' size mismatch: nbytes=%lld but "
                        "numel=%lld x dtype_size=%d = %lld (shape/dtype/data_offsets disagree)\n",
                        files[fi], name, (long long)nbytes, (long long)numel, st_dtype_size(dtype),
                        (long long)want_nbytes);
                exit(1);
            }
            if (data_start + b0 > file_size) {
                fprintf(stderr, "st_init: %s: tensor '%s' data_offsets end (%lld) exceeds file size (%lld)\n",
                        files[fi], name, (long long)(data_start + b0), (long long)file_size);
                exit(1);
            }
