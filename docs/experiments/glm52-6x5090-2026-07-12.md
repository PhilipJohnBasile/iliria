...
This note records the July 12, 2026 experiments that compared iliria with
...
The day had two halves. The morning runs (below) measured iliria at 0.12
tok/s and vLLM-Moet at 2.3-2.7 tok/s. The afternoon full-resident work on
iliria (see "iliria full-resident experiments") removed disk from the decode
...
- iliria INT4 model: `/data/models/GLM-5.2-int4` (144 shards)
...
| --- | --- | ---: | ---: | --- |
| iliria INT4 (morning, partial residency) | 6 GPUs | about 42 s | 0.12 tok/s | GPU hot tier was active, but dense compute remained on CPU |
| iliria INT4 (afternoon, full residency) | 6 GPUs + 24 cores | — | 6.28-6.84 tok/s | all 19,456 experts resident in VRAM+RAM, zero disk waits |
...
The morning iliria run generated 16 tokens in 132.46 seconds. Its measured
...
## iliria full-resident experiments (afternoon)

Test directory: `/data/test/iliria-full-resident`. Related PR:
...
Logs: `/data/test/iliria-six-gpu-ab-mtp-*-20260712.log` (phase 1),
`/data/test/iliria-six-gpu-ab-mtp8-*-20260713.log` (phase 2).
...
`/data/test/iliria-six-gpu-ab-avx512q-*-20260713.log`,
...
model fits in VRAM+RAM, and once disk left the decode path iliria at
6.28-6.84 tok/s beat every vLLM-Moet layout measured here (best 2.6 tok/s),
because iliria never replays a whole step on an expert miss.
...
The fastest configuration overall is iliria full-resident in
`/data/test/iliria-full-resident` (6.28-6.84 tok/s; stop the
...
reproduced. Raw A/B logs live under `/data/test/iliria-six-gpu-ab-*.log`.
